...
export const createNode = (mode: viewMode, filename: string, linked_files: string[], type: string): Node | null => {
  ...
  for (const linkedFile of linked_files) {
    createLink(mode, newId, linkedFile, type);
  }
...
export const createLink = (mode: viewMode, source: number | string, target: number | string, type: string): boolean => {
  const graphData = readGraphJson(mode);
  if (!graphData) {
    return false;
  }
  ...
  if (typeof source === 'string') {
    const sourceNode = getNode(mode, source);
    if (!sourceNode) {
      const newSourceNode = createNode(mode, source, [], type);
  ...
  if (typeof target === 'string') {
    const targetNode = getNode(mode, target);
    if (!targetNode) {
      const newTargetNode = createNode(mode, target, [], type);
...
export const getNode = (mode: viewMode, filename: string): Node | null => {
  const graphData = readGraphJson(mode);
  if (!graphData) {
    return null;
  }
...
export const deleteNode = (mode: viewMode, id?: number, filename?: string): boolean => {
  const graphData = readGraphJson(mode);
  if (!graphData) {
    return false;
  }
  ...
  let nodeId = id;
  if (!nodeId && filename) {
    const node = getNode(mode, filename);
    if (node) {
      nodeId = node.id;
    } else {
...
export const deleteLink = (mode: viewMode, source: number | string, target: number | string): boolean => {
  const graphData = readGraphJson(mode);
  if (!graphData) {
    return false;
  }
...
export const getLinks = (mode: viewMode): Link[] | null => {
  const graphData = readGraphJson(mode);
  if (!graphData) {
    return null;
  }
...
export const parseFileLinks = (content: string, availableFiles: string[]): string[] => {
